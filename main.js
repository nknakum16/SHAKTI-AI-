...
let messageIndex = [];
let userId = null;
let chatHistory = []; // Add chat history array
...
class ChatManager {
    constructor() {
        ...
        this.currentTopic = 'New Conversation';
        this.currentIcon = 'bi-chat-dots';

        this.setupEventListeners(); 
        this.initializeUI();
        this.setupFileUpload();
        this.loadCurrentTopic(); // Load current topic on initialization
    }
    ...
    async sendMessage() {
        ...
        // Save user message
        saveConversation(message, 'user', userId);
        ...
        try {
            ...
            if (response && response.success && response.response) {
                ...
                // Save AI message
                saveConversation(response.response, 'assistant', userId);
                this.addMessageToChat(response.response, 'ai');

                // Extract topic after a few messages
    ...
    async extractTopic() {
        try {
            ...
            if (!userId) {
                console.log('No userId available for topic extraction');
                return;
            }

            console.log('Extracting topic for user:', userId);
            ...
            const data = await response.json();
            console.log('Topic extraction response:', data);
            
            if (data.success && data.topic) {
                this.currentTopic = data.topic;
                this.currentIcon = data.icon;
                this.updateSidebarTopic();
                console.log('Topic updated:', data.topic, 'Icon:', data.icon);
            } else {
                console.log('Topic extraction failed:', data.error);
            }
        } catch (error) {
    ...
    async loadCurrentTopic() {
        if (!userId) {
            console.log('No userId available for loading topic');
            return;
        }

        try {
            console.log('Loading current topic for user:', userId);
            
            const response = await fetch('extract_topic.php', {
                method: 'POST',
                headers: {
            ...
            const data = await response.json();
            console.log('Load topic response:', data);
            
            if (data.success && data.topic) {
                this.currentTopic = data.topic;
                this.currentIcon = data.icon;
                this.updateSidebarTopic();
                console.log('Loaded topic:', data.topic, 'Icon:', data.icon);
            } else {
                console.log('No saved topic found, using default');
            }
        } catch (error) {
    ...
    updateSidebarTopic() {
        // Find the active chat item in the sidebar using the ID
        const activeChatItem = document.getElementById('currentChatItem');
        if (activeChatItem) {
            const iconElement = activeChatItem.querySelector('i');
            const textElement = activeChatItem.querySelector('span');
            
            if (iconElement) {
                iconElement.className = this.currentIcon;
            }
            
            if (textElement) {
                textElement.textContent = this.currentTopic;
            }
            
            console.log('Sidebar updated with topic:', this.currentTopic, 'Icon:', this.currentIcon);
        } else {
            console.log('Could not find currentChatItem in sidebar');
        }
    }

    // Manual topic extraction for testing
    async manualExtractTopic() {
        console.log('Manual topic extraction triggered');
        await this.extractTopic();
    }
...
document.addEventListener('DOMContentLoaded', () => {
    fetch('config.php')
        .then(res => res.json())
        .then(data => {
            if (data.status === 'success' && data.default_model) {
...
function saveConversation(message, role, user_id = null) {
...
// Global function for manual topic extraction testing
function testTopicExtraction() {
    if (window.chatManager) {
        window.chatManager.manualExtractTopic();
    } else {
...
console.log('Current topic:', window.chatManager?.currentTopic);

// Manual extraction
testTopicExtraction();

// Check database
fetch('test_topic_db.php').then(r => r.text()).then(console.log);
