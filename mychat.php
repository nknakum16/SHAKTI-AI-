...
";
    ...
    private function select_optimal_models($query_analysis) {
        ...
        $preferred_model = isset($query_analysis['preferred_model']) ? $query_analysis['preferred_model'] : null;
        ...
        return array_slice($selected_models, 0, 7, true);
    }

    private function get_priority_score($priority) {
        $scores = ['high' => 3, 'medium' => 2, 'low' => 1];
        return isset($scores[$priority]) ? $scores[$priority] : 1;
    }
    ...
    private function get_model_display_name($model) {
        $display_names = [
            'chatgpt-40-latest' => 'GPT-4',
            'gemini-1.5-flash' => 'Gemini Flash',
            'openai-reasoning' => 'OpenAI Reasoning',
            'blackboxai-pro' => 'BlackBox Pro',
            'qwen-coder' => 'Qwen Coder',
        ...
        return isset($display_names[$model]) ? $display_names[$model] : ucfirst(str_replace('-', ' ', $model));
    }

    private function truncate_response($text, $max_length) {
    ...
    public function send_message($user_message, $model = "shakti") {
        try {
            ...
            if ($model !== 'shakti') {
                $query_analysis['preferred_model'] = $model;
            }
            ...
            if (empty($responses)) {
                throw new Exception("No valid responses received from any model");
            }
            ...
            // After successful processing of a message (i.e., after sending a response), increment the API calls today counter
            ...
            if (!$conn->connect_error) {
                $today = date('Y-m-d');
                $conn->query("INSERT INTO api_stats (stat_date, calls_today) VALUES ('$today', 1) ON DUPLICATE KEY UPDATE calls_today = calls_today + 1");
                $conn->close();
            }
    ...
    public function get_model_stats() {
        ...
        foreach ($this->model_config as $model => $config) {
            foreach ($config['specialty'] as $specialty) {
                $specialties[$specialty] = (isset($specialties[$specialty]) ? $specialties[$specialty] : 0) + 1;
            }
        }
...
try {
    ...
    if (!$data || !isset($data['message'])) {
        throw new Exception('Invalid input format - message field required');
    }
    ...
    // Clean any remaining output buffer and send JSON
    ob_clean();
